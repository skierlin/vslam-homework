...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
		...
		int x = kp.pt.x;
		int y = kp.pt.y;
		if(x>=8 && x<=(image.cols-8) && y>=8 && y<(image.rows-8)){
			for(int i = -8; i < 8; i++){
				for(int j = -8; j < 8; j++){
					m01 += i*image.at<uchar>(y+j,x+i);
					m10 += j*image.at<uchar>(y+j,x+i);
				}
			}
			kp.angle = (float)atan2(m01,m10)*180.0/pi; // compute kp.angle
		}
		else
			kp.angle = 0; 
		// END YOUR CODE HERE
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
	for (auto &kp: keypoints) {
		...
		for (int i = 0; i < 256; i++) {
			// START YOUR CODE HERE (~7 lines)
			int up = ORB_pattern[4*i];
			int vp = ORB_pattern[4*i+1];
			int uq = ORB_pattern[4*i+2];
			int vq = ORB_pattern[4*i+3];
			int upp = round(std::cos(kp.angle / 180 * pi) * up - std::sin(kp.angle / 180 * pi) * vp) + kp.pt.x;
			int vpp = round(std::sin(kp.angle / 180 * pi) * up + std::cos(kp.angle / 180 * pi) * vp) + kp.pt.y;
			int uqq = round(std::cos(kp.angle / 180 * pi) * uq - std::sin(kp.angle / 180 * pi) * vq) + kp.pt.x;
			int vqq = round(std::sin(kp.angle / 180 * pi) * uq + std::cos(kp.angle / 180 * pi) * vq) + kp.pt.y;

			if (upp < 0 || vpp < 0 || uqq < 0 || vqq < 0 || upp > image.cols - 1 || uqq > image.cols - 1 || vpp > image.rows - 1 || vqq > image.rows - 1)
			{
				d.clear();
				break;
			}
			d[i] = image.at<uchar>(vpp, upp) > image.at<uchar>(vqq, uqq) ? 0 : 1;  // if kp goes outside, set d.clear()*/            
		// END YOUR CODE HERE
		}
		desc.push_back(d);
	}
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
	...
	// END YOUR CODE HERE
    DescType result(256,false);
    for(int i = 0; i< desc1.size(); i++){
        int minDistance = 256;
        int minIndex = 0;
        for(int j = 0; j < desc2.size(); j++){
            for(int k = 0; k < desc1[i].size(); k++){
                result[k] = desc1[i][k]^desc2[j][k];
            }
            int distance = 0;
            for(int m = 0; m < result.size(); m++){
                distance += (result[m]!=0)?1:0;
            }
